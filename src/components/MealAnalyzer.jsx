  ...
  return (
    <div className="bg-slate-800 rounded-2xl shadow-xl border border-slate-700 overflow-hidden mb-8 animate-fade-in-up">
      <div className="p-6 md:p-8">
        ...
        {!imagePreview ? (
          <div 
            className="border-2 border-dashed border-slate-600 hover:border-emerald-500/50 rounded-2xl p-12 text-center transition-colors cursor-pointer group"
            onClick={() => fileInputRef.current?.click()}
          >
            <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-slate-700 text-slate-400 group-hover:text-emerald-400 group-hover:bg-emerald-500/10 mb-4 transition-colors">
              <Upload size={28} />
            </div>
            <h3 className="text-lg font-medium text-white mb-2">Upload or Take a Photo</h3>
            <p className="text-slate-400 text-sm max-w-xs mx-auto">
              Snap a picture of your meal to get an instant nutritional breakdown using Gemini AI.
            </p>
          ...
          <div className="space-y-6">
            <div className="relative rounded-2xl overflow-hidden bg-slate-900 border border-slate-700 aspect-video md:aspect-[21/9] flex items-center justify-center">
              ...
              {!isAnalyzing && (
                <button 
                  onClick={resetImage}
                  className="absolute top-4 right-4 bg-slate-900/80 backdrop-blur text-slate-300 hover:text-white px-3 py-1.5 rounded-lg text-sm font-medium border border-slate-700 transition-colors"
                >
                  Change Photo
                </button>
              )}
